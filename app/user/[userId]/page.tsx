...
export default function UserPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("profiles").select("display_name, avatar_url, bio")
      .eq("id", userId).single()
  ...
  const S = {
    ...
    name: {
      fontWeight: 700, fontSize: "1.5rem", letterSpacing: "-0.02em",
      color: "#F0EDE8", margin: 0,
    },
    sub: { fontSize: "0.9375rem", color: "rgba(240,237,232,0.45)", margin: 0 },
    wordmark: {
      fontWeight: 800, fontSize: "0.8125rem", letterSpacing: "0.12em",
      textTransform: "uppercase" as const, color: "rgba(240,237,232,0.3)",
    },
    btn: {
    ...
    <>
      ...
      <main style={S.page}>
        <div style={S.inner}>
          ...
          {profile?.avatar_url
            ? <img src={profile.avatar_url} alt={profile.display_name ?? ""} style={S.avatar} />
